...
@jit(nopython=True)
def tc_joint(osc):
    tc_r = np.add(np.multiply(100, osc), 90)
    tc_l = np.add(np.multiply(100, -osc), 90)
...
def ctr_joint(osc, offset):
    factor = 100-offset
    ctr_r = np.add(np.multiply(factor, osc), 90+offset)
    ctr_l = np.add(np.multiply(factor, osc), 90-offset)
